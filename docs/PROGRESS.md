...
| 3 | The core contribution: context-conditioned detection + ClickHouse + Iceberg | Measured false-positive reduction vs. the unconditioned baseline; fail-open verified | **Gate clauses satisfied; the requirement behind them is still not met, and after nine measurements the reason has changed.** Nine runs published, NFR-8 missed in all nine: context conditioning alone tops out at +18.9%/-3.3% (v4, 12 ch) and +3.6%/-3.3% (v4w, 24 ch), and B-6 computed why -- two thirds of the false pages have no attributable cause. **Adding cross-detector agreement (ADR-050) took it to +24.0%/+0.0% at 12 channels and +35.4%/-3.3% at 24**, removing 52% of the un-attributable pages B-6 had counted as out of reach, and at 24 channels improving *both* halves at once for the first time in the project. Isolated by a fourth ablation pass in each run: worth 18.0 and 25.7 points of reduction respectively. Fail-open verified live in every run, 145/145 in the largest. The plausibility half works throughout; what is left of the gap is about six false pages that **both** detectors see and neither can tell moved for no reason. **ClickHouse and Iceberg are now built and running** (ADR-043 to ADR-046): the serving store holds readings and window scores, the lake holds the durable record on MinIO, and the reconciliation harness audits against it -- ledger and lake agreed on 6/6 channels from two independently derived counts |
...
| D2 | Deploy-marker conditioning | **Measured nine times, all nine published, NFR-8 missed in all nine.** v1 (co-occurrence) +60.9%/-36.7%; v2 (synchrony on quantised starts) +9.0%/-10.0%; low density +6.7%/-6.7%; v3 (synchrony on true onsets) +11.1%/-6.7%; v4a (evidence made present, ADR-037) +27.8%/-16.7%; v4 (blast radius, ADR-038) +18.9%/-3.3%; v4w at 24 ch +3.6%/-3.3%; **v6a (cross-detector agreement, ADR-050) +24.0%/+0.0%; v6aw at 24 ch +35.4%/-3.3%**. The trajectory is the finding: four runs argued about timing resolution, one about topology, and B-6 then computed that two thirds of the target population was outside the reach of all of them. The signal that finally moved the number consults no context event at all -- and the second detector raises no episodes, so the shadow pass and the false-page denominator are unchanged |
...
| When | Commit | What |
|---|---|---|
| 2026-09-18 | (this commit) | **v6 measured at two fleet widths: the un-attributable false pages turned out to be reachable, and NFR-8 is still missed.** B-6 had computed that two thirds of the false pages overlap no injected excursion, so no context signal could touch them. That was right about context conditioning and wrong as a ceiling on conditioning: the platform already runs a second detector on the same windows, and asking it needs no deploy and no pipeline event. **+24.0%/+0.0% at 12 channels, +35.4%/-3.3% at 24**, against v4's +18.9%/-3.3% and v4w's +3.6%/-3.3% -- and at 24 channels **both halves improved at once for the first time in the project**, reduction up 25.7 points and recall loss halved, because agreement pulled back ten of the eleven deploy attributions and seven of those overlapped a real fault. Isolated by a fourth pass in which the model runs, is waited for, records what it saw and may not act (ADR-051) -- an ablation that dropped the model would have put the barrier's timing change inside the measured contribution. **Three things this refused to do.** It does not let the second detector raise episodes, because the shadow pass and the false-page denominator would both move and nothing would be comparable to v1-v4. It does not read silence as dissent: cold start, a dropped window and an absent model all abstain and raise the episode, which was 20% of decisions at 12 channels and is reported rather than absorbed. And it fixed the agreement line at 3.0 before the run, below the model's own alarm threshold, publishing the whole sensitivity curve beside the result -- at a bar of 6.0 the reduction would have been larger, so the chosen line is not the flattering one. Costs reported at both resolutions: 2 true pages and 0 incidents at 12 channels, 6 and 1 at 24, quiet-window recall 7/7 in both, fail-open 76/76 and 145/145. G-17 and G-18 record what these runs cannot establish. |
...
restated or the window geometry changes. **B-6 is largely answered by v6, by the option nobody
recommended**: the un-attributable pages were reduced without touching the detector, by
consulting the second one, so there is no longer any need to redefine the denominator to make
the number interesting. What is left of B-6 is a protocol question -- the agreement line sits
six false pages from the target and is exactly the knob that would close it, so whether a sweep
of that line counts as calibration or as tuning is the architect's call. The implementer's view
is that any sweep must be scored on a different seed from the one it was chosen on.
...
**B-4 is answered again by v6, and the answer changed.** v4's diagnosis -- a distributed-systems
defect rather than a statistical one -- was right about what was broken and wrong about where
the headroom was. Context conditioning cannot reach a false page with no attributable cause, and
most of them have none; cross-detector agreement needs no cause at all (ADR-050). At 24 channels
it took the run from +9.7%/-6.7% to **+35.4%/-3.3%** on byte-identical records -- the first time
any mechanism here improved both halves of NFR-8 at once, because agreement pulled back ten of
the eleven deploy attributions and seven of those overlapped a real fault. **NFR-8 is still
missed, for the sixth and seventh time**, by about six false pages rather than by a ceiling.
Sections 3.12 to 3.14 of `docs/EVALUATION.md`.

**The v4 answer, kept as written.** The root cause was distributed-systems
